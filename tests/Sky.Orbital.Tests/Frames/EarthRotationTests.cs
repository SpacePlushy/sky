...
{
    // The example is stated as UTC 2004-04-06 07:51:28.386 with UT1-UTC = -0.439961 s. The
    // reference computation held UT1 as one double, JD(UTC) + dUT1/86400, which rounds twice
    // and lands on 2453101.8274067831225693..., printed in the paper as 2453101.82740678310.
    // That value is 22.79 microseconds after the exact instant, because a double near JD 2.45e6
    // resolves only 40.2 microseconds. To compare like with like, these tests give Sky that same
    // instant. Sky's split Julian date represents it exactly (the subtraction below is exact).
    ...
    private static readonly DateTimeOffset ExampleUtc =
        new DateTimeOffset(2004, 4, 6, 7, 51, 28, TimeSpan.Zero).AddTicks(3_860_000);

    private const double ExampleUt1MinusUtcSeconds = -0.439961;
    ...
    public void Matches_the_published_itrf_velocity_once_polar_motion_is_applied()
    {
        ...
        // Published to 1e-9 km/s. Tolerance 0.1 mm/s. Expect about 0.06 mm/s in x: Sky subtracts
        // Earth rotation at the GMST rate so velocity is exactly the derivative of position, while
        // the paper uses the inertial rate, lower by 7.1e-12 rad/s. At y = 7901 km that is 0.056 mm/s.
        AssertClose(new Vec3(-3.225636520, -2.872451450, 5.531924446), itrf, 1e-7);
    }

    [Fact]
    public void Split_julian_date_keeps_the_instant_the_reference_double_shifts_by_22_79_microseconds()
    {
        ...
        var exactUt1 = JulianDate.FromInstant(ExampleUtc.AddTicks(-4_399_610)); // UTC - 0.439961 s
        ...
        Assert.Equal(22.790, microseconds, 0.05);
    }
    ...
    {
        // Assumption A5. Leaving out the -0.439961 s correction rotates the Earth 0.439961 s too far:
        // 7.29212e-5 rad/s * 0.439961 s * 7968.6 km (distance from the spin axis) = 0.2557 km.
